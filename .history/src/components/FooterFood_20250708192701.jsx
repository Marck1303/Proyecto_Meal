...
export default function FooterFood() {
  ...
  return (
    <footer className="bg-[#fbc81d] font-bold text-black py-6 shadow-inner text-center">
      <h3 className="font-bold text-lg mb-4">Equipo Creativo: DesignFront</h3>
      ...
      <div className="flex flex-wrap justify-center gap-30">
        {miembrosDelGrupo.map((miembro) => (
          ...
          >
            <img
              src={miembro.img || "/default-avatar.png"}
              alt={miembro.nombre}
              className="w-16 h-16 rounded-full mb-2 border border-gray-300 shadow"
            />
            <span className="font-semibold">{miembro.nombre}</span>
          </a>
        ))}
      </div>
    </footer>
