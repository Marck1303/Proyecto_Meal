...
export default function SearchBar({ buscarT, setBuscarT, buscarFotos }) {
  const [inputValue, setInputValue] = React.useState(buscarT);
  const buscarFotos = async () => {};

  React.useEffect(() => {
    setInputValue(buscarT);
  }, [buscarT]);

  const handleSearch = () => {
    buscarFotos(inputValue);
  };
